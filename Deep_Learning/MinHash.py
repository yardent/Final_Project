...
dataFile = "/home/mapr/PycharmProjects/Deep_Learning/data/the-wolf-of-wall-street-fix-no-low.txt"
# # predictFile = "/home/mapr/PycharmProjects/Project_OfritSigal/results/the-wolf-of-wall-street-predict.txt"
predictFile = "/home/mapr/PycharmProjects/Deep_Learning/results/Output-oran-last-no-low-new.txt"
...
count = 0
fileCount = 0
listOfFiles = [predictFile, dataFile]
...
for file in listOfFiles:
    ...
    for line in f:
        if line != "\n":
            if len(line.split(" ")) > 1:
                fileList.append(str(count) + " " + line)
                count += 1
                fileCount += 1
    fileSizeList.append(fileCount)

predictSize = fileSizeList[1] - fileSizeList[0]
...
for i in range(0, numDocs):
  ...
  dic_word_to_sent[docID] = ' '.join(words)
  del words[0]
    ...
    for i in range(0, numDocs):
      ...
      for j in range(i + 1, numDocs):
        ...
        if len(s1.union(s2))> 0:
            JSim[get_triangle_index(i, j)] = (len(s1.intersection(s2)) / len(s1.union(s2)))
        else:
            JSim[get_triangle_index(i, j)] = 0
...
for i in range(0, numDocs):
  ...
  for j in range(i + 1, numDocs):
    ...
    estJSim[get_triangle_index(i, j)] = (count / numHashes)
...
for i in range(0, numDocs):
  for j in range(i + 1, numDocs):
    ...
    if estJ > threshold:
      ...
      if J > 0 and (int(docNames[i]) < int(fileSizeList[0]) and int(docNames[j]) >= int(fileSizeList[0]) or
                                int(docNames[i]) >= int(fileSizeList[0]) and int(docNames[j]) < int(fileSizeList[0])):
          print "  %5s --> %5s   %.2f     %.2f   %5s    %5s" % (docNames[i], str(int(docNames[j]) - int(fileSizeList[0])), estJ, J, dic_word_to_sent[docNames[i]], dic_word_to_sent[docNames[j]])

# Display true positive and false positive counts.
print

print "numDocs: " + str(numDocs)
print "predictSize: " + str(predictSize)
